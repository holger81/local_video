...
- **Storyboard:** `propose_storyboard` (safe; `target_duration_sec` / `avg_beat_sec`), `replace_storyboard`, `list_frames` / `get_frame` / `create_frame` / `delete_frame`, `update_frame` (incl. `audio_notes`), `approve_storyboard`, `generate_frame_dialog` / `batch_generate_dialogs`, `generate_cast_ref_sheet`, stills/keyframes/step-clips/between-stills (+ edit/delete helpers), `start_keyframes_job`, `set_frame_still_from_media`, `set_keyframe_from_media`
