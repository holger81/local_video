...
- **Scenery:** `list_scenery`, `create_scenery`, `update_scenery` (incl. variants / `reference_image_path`), `delete_scenery`, `generate_scenery_reference`, `delete_scenery_reference`, `set_scenery_reference_from_media`, `generate_scenery_variant_reference`, `set_scenery_variant_reference_from_media` — assign on beats via `update_frame` / `replace_storyboard` `scenery: [{scenery_id, variant_id}]`
- **Storyboard:** `propose_storyboard` (safe; `target_duration_sec` / `avg_beat_sec`), `replace_storyboard`, `list_frames` / `get_frame` / `create_frame` / `delete_frame`, `update_frame` (incl. `audio_notes`, `cast`, `scenery: [{scenery_id, variant_id}]`), `approve_storyboard`, `generate_frame_dialog` / `batch_generate_dialogs`, `generate_cast_ref_sheet`, stills/keyframes/step-clips/between-stills (+ edit/delete helpers), `rebuild_frame_keyframe_prompts` (`spacing_sec` default ~5; pass `2` for denser FLF2V beats), `start_keyframes_job`, `set_frame_still_from_media`, `set_keyframe_from_media`. Empty `cast=[]` means no people on first **and** mid/last; assign scenery on establishing beats so mid/last re-lock location geometry.
